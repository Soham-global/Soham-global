<div align="center">

[![Typing SVG](https://readme-typing-svg.demolab.com?font=Fira+Code&size=32&duration=3500&pause=1000&color=00FFFF&center=true&vCenter=true&width=800&lines=Hi+there%2C+I'm+Soham!;AI+Engineer;RAG+Pipelines+%7C+Multi-Agent+Systems;LangChain+%7C+AutoGen+%7C+LLMs)](https://git.io/typing-svg)

</div>

---

## 👋 About Me

🤖 **AI Engineer** specialized in building **RAG apps** and **multi-agent systems**

🚀 Hands-on experience building LLM-driven agentic systems, GenAI workflows, RAG pipelines, and multi-agent applications — skilled in LangChain, AutoGen, prompt engineering, vector databases, and Python.

🎓 B.Tech in Computer Science, Guru Nanak Dev University

📄 Reach me at **magiciancoder1@gmail.com**

---

## 🚀 Tech Stack

### 🧠 GenAI & Agentic AI
`LangChain` `LangGraph` `Microsoft AutoGen` `RAG Pipelines` `Multi-Agent Systems` `Prompt Engineering` `Semantic Search`

### 🔮 LLMs & Inference
`LLaMA 3.3 70B` `Mistral 7B` `Groq API` `OpenRouter API` `Streaming (SSE)`

### 🗂️ Vector Databases
`ChromaDB` `Pinecone` `FAISS`

### 👁️ Computer Vision
`YOLOv5` `OpenCV` `Object Detection` `Transfer Learning` `CNNs`

![Skills](https://skillicons.dev/icons?i=py,pytorch,tensorflow,opencv,fastapi,flask,postgres,mysql,cpp,docker,git,aws)

---

## 📊 GitHub Stats

<div align="center">

![Soham's GitHub Stats](https://github-readme-stats.vercel.app/api?username=Soham-global&show_icons=true&theme=tokyonight&hide_border=true)

![GitHub Streak](https://streak-stats.demolab.com/?user=Soham-global&theme=tokyonight&hide_border=true)

![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=Soham-global&layout=compact&theme=tokyonight&hide_border=true)

</div>

---

## 💡 Currently

- 🔭 I'm currently working on **RAG pipelines** and **multi-agent AI systems**
- 🌱 I'm currently exploring **LangGraph** and advanced **LLM orchestration**
- 🤝 I'm looking to collaborate on **GenAI / Agentic AI** projects
- 💬 Ask me about **RAG**, **LangChain**, **AutoGen**, or **LLM-based applications**
- 📫 How to reach me: **magiciancoder1@gmail.com**

---

## 🌐 Let's Connect

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Connect-blue?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/sohamkalsi/)
[![Email](https://img.shields.io/badge/Email-Contact-red?style=for-the-badge&logo=gmail&logoColor=white)](mailto:magiciancoder1@gmail.com)
[![GitHub](https://img.shields.io/badge/GitHub-Follow-black?style=for-the-badge&logo=github&logoColor=white)](https://github.com/Soham-global)

</div>
